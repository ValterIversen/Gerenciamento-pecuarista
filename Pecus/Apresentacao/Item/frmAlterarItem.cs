...
namespace Apresentacao.Item
{
    public partial class frmAlterarItem : Form
    {
        ...
        private void BtnSalvar_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                try
                {
                    ...
                    if (itemNegocios.ValidarExist(item.ItemID, txtDescricao.Text.Trim()).Count > 0)
                        MessageBox.Show("Já existe um item com esta descrição cadastrado.", "Duplicação de item.");
                    else
                    {
                        itemNegocios.Alterar(item);
                        MessageBox.Show("Item de ID: " + item.ItemID + " cadastrado com sucesso!");
                        Close();
                    }
        ...
        private void FrmAlterarItem_Load(object sender, EventArgs e)
        {
            ...
            txtDescricao.Text = item.ItemDescricao;
            cbbTipoItem.SelectedValue = item.TipoItem.TipoItemID;
        }
    }
}
